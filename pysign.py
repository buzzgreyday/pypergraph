...
if __name__ == "__main__":
    ...
    signature = coincurve_sig(base64.b64decode("HGbib2kv9gm9IJjDt1FXbXFczZi35u0rZR3iPUIt5GglDDCeIQ7v8eYXVNIaLoJRI4URGZrhwmsYQ9aVtRTnTfQ="))
    encoded = _zero_format(bytes("hello", "utf8"))
    s = time.time()
    for i in range(1000):
        publickey = recover_public_key(signature, encoded)
        address = compute_public_address(publickey)
    assert address == b'1N2XWu5soeppX2qUjvrf81rpdbShKJrjTr'
    print("- Verify x1000: %.3fs" % (time.time() - s))
    ...
    assert compute_public_address(recover_public_key(coincurve_sig(sign), encoded)) == b'1N2XWu5soeppX2qUjvrf81rpdbShKJrjTr'
