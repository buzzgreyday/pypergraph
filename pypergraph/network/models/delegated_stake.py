...
class DelegatedStakesInfo(BaseModel):
    address: str
    active_delegated_stakes: List[DelegatedStakeInfo] = Field(alias="activeDelegatedStakes")
...
class NextDagPrice(BaseModel):
    ...
    as_of_epoch: int = Field(description="Progress throughout epochs used for rewards minting")
...
class DelegatedRewardsInfo(BaseModel):
    ...
    current_dag_price: int = Field(description="Current DAG price in datum/USD format", alias="currentDagPrice")
    next_dag_price: NextDagPrice = Field(alias="nextDagPrice")
    total_delegated_amount: int = Field(description="Total amount delegated across all active delegated stakes, including accrued rewards", alias="totalDelegatedAmount")
    latest_average_reward_per_dag: int = Field(description="Average reward rate per DAG per epoch in datums", ge=0, alias="latestAverageRewardPerDag")
    total_dag_amount: int = Field(description="Current total supply of all DAG balances in datums", alias="totalDagAmount")
    total_rewards_per_latest_epoch: int = Field(description="Total incremental rewards added in the latest epoch in datums", ge=0)
    total_rewards_per_year_estimate: int = Field(description="Total reward APY per DAG in datums (latestAverageRewardPerDag * epochsPerYear)")
